...
def plot2d(out, x_vals):
    ...
    for i in range(out.shape[0]):
        ...
        # sorted_arr = np.sort(out[i])
        # second_largest = sorted_arr[-42]
        # second_smallest = sorted_arr[41]
...
def create_block(
    ...
    if layer_idx not in attn_layer_idx:
        ...
        mixer_cls = MHA(layer_idx=layer_idx, **attn_cfg, **factory_kwargs)
    
        #block.register_forward_hook(block.get_activation(name, block.layer_idx))
    norm_cls = partial(
    ...
    # for fullname, _ in block.named_parameters():
