...
def plot_activation(output):
    ...
    # 중앙값을 기준으로 75% 및 99% 퍼센타일 범위를 계산
    
    
    for i in range(out.shape[0]):
        percentile_75_upper = np.percentile(out[i], 75)  # 중앙값 위쪽에서 75%
        percentile_99_upper = np.percentile(out[i], 99) # 중앙값 위쪽에서 99%
        percentile_25_lower = np.percentile(out[i], 25)  # 중앙값 아래쪽에서 25% (즉, 75%)
        percentile_99_lower = np.percentile(out[i], 1)
        
        min_val = np.min(out[i])
        min_index = np.argmin(out[i])
        
    # 중앙값 기준 아래쪽 99% 이하 (보라색)
        ...
        # 중앙값 기준 아래쪽 99% ~ 25% 사이 (연한 파란색)
        # 중앙값 기준 25% ~ 75% 사이 (노란색)
        
        # 중앙값 기준 75% ~ 99% 사이 (연한 파란색)


        # 중앙값 기준 위쪽 99% 이상 (보라색)
        
        
    # #Y = np.arange(range_min, range_max)
    # colors = plt.get_cmap('GnBu')
    # plt.figure(figsize=(8, 6))
    # plt.plot(out.T, alpha=0.3, cmap=colors) 
    # 그래프 스타일 설정
    plt.xlabel("channel")
    plt.ylabel("values")
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        for name, tensor in self.self_attn.named_parameters():
            self.self_attn.register_forward_hook(self.self_attn.get_activation(name))
            
        for name, tensor in self.mlp.named_parameters():
            self.mlp.register_forward_hook(self.mlp.get_activation(name))
        
        self.input_layernorm.register_forward_hook(self.input_layernorm.get_activation('input_layernorm'))
        self.post_attention_layernorm.register_forward_hook(self.post_attention_layernorm.get_activation('post_attention_layernorm'))
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.post_init()
        self.norm.register_forward_hook(self.norm.get_activation('norm'))
